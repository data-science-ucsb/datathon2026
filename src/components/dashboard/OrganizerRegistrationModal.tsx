...
const OrganizerRegistrationModal: React.FC<OrganizerRegistrationModalProps> = ({ registration, onClose }) => {
    ...
    >
      ...
      >
        ...
        <div className="space-y-4">
          ...
          {registration.applicationCompleted && (
            <>
              <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <span className={`inline-block mt-1 px-3 py-1 rounded-full text-sm font-medium ${getStatusBadgeClass(registration.status)}`}>
                {registration.status}
              </span>
            </div>
            <div>
              <p className="text-sm text-gray-500">Role</p>
              <span className={`inline-block mt-1 px-3 py-1 rounded-full text-sm font-medium ${getRoleBadgeClass(registration.role)}`}>
                {registration.role}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-lg font-medium">{new Date(registration.createdAt).toLocaleString()}</p>
          </div>

          <div>
            <p className="text-sm text-gray-500">User ID</p>
            <p className="text-sm font-mono text-gray-400">{registration.uid}</p>
          </div>
        </div>
      </div>
    </div>
...
export default OrganizerRegistrationModal;
